...
def main():
    ...
    for fold_idx in range(args.num_folds):
        ...
        if test_ood_loader is not None:
            ...
            test_ood = False
        logdir=f"runs/{os.path.basename(args.outdir)}_{fold_idx}/{datetime.datetime.now().strftime('%Y%m%d_%H-%M-%S')}"
        ...
        for epoch in trange(args.num_epochs, desc="epochs", leave=False):
            ...
            for i, (data, labels) in enumerate(tqdm(train_loader, desc="train", leave=False)):
                ...
                emb = model(data)
                # mean = torch.mean(emb[labels == 0], dim=0)
                # dists = torch.cdist(emb.reshape(1, *emb.shape), mean.reshape(1, 1, *mean.shape)).squeeze()
                dists = torch.mean(emb**2, dim=-1)
                pull_loss = torch.mean(
                    torch.max(
                        dists[labels == 0] - model.r,
                        torch.zeros_like(dists[labels==0])
                    )
                )
                if torch.count_nonzero(labels == 1) != 0:
                    push_loss = torch.mean(
                        torch.max(
                            model.r - dists[labels == 1] + args.alpha,
                            torch.zeros_like(dists[labels==1])
                        )
                    )
                else:
                    push_loss = torch.zeros((1,)).cuda()

                loss = args.reg_lambda * pull_loss + push_loss + 0.001 * torch.abs(model.r - 0.5)
            ...
            with torch.no_grad():
                ...
                for data, labels in tqdm(test_id_loader, desc="Test-Classification", leave=False):
                    ...
                    predicted = torch.where(dists > model.r.item(), 1, 0)
                    total += labels.size(0)
            ...
            writer.add_scalar("Param/r", model.r.item(), epoch)
